 ...
 */
function gitChangeSummary(cwd, jobDir, startedAt) {
...
function cmdFinalize(jobDir, exitCode) {
  ...
  const summary = gitChangeSummary(job.cwd, jobDir, job.startedAt);
  if (summary) {
...
function cmdAgentRead(file) {
  let agent;
  try { agent = JSON.parse(fs.readFileSync(file, 'utf8')); }
  catch (e) { console.error('agent-read: ' + e.message); process.exit(1); }
  process.stdout.write(`${agent.model || ''}\n${agent.worktree ? '1' : '0'}\n${agent.variant || ''}\n`);
}
...
module.exports = { diffStatusLines, pad, gitChangeSummary };
