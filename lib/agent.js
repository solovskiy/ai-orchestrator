...
function cmdShow(jobDir) {
  ...
  if (j.interactive) {
    row('режим', j.status === 'running'
      ? 'интерактивный — agent talk ' + j.id + ' "<текст>"'
      : 'интерактивный (сессия не активна)');
    if (j.interactiveDegraded) row('  ⚠', 'fifo недоступен — запущен как обычная задача');
  }
