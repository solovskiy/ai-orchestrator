...
function main() {
  ...
  if (targetDir) {
    ...
    // freshness-check: не перезаписывать, если .md новее или равен .json
    let outMtime = 0;
    try { outMtime = fs.statSync(outFile).mtimeMs; } catch {} // файла может не быть
    const srcMtime = fs.statSync(jsonFile).mtimeMs;
    if (outMtime >= srcMtime) {
      console.log('deploy-agent: ' + outFile + ' уже актуален, пропускаю');
      return;
    }
