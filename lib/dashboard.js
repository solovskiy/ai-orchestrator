 ...
 * capability. Слушает только localhost (127.0.0.1).
 *
...
const MEMORY_FILE = path.join(AI_HOME, 'memory', 'index.json');
...
/** Преобразует плоский объект памяти в массив entries для API */
function memoryToEntries(data) {
...
function apiStats(req, res) {
  ...
  json(res, {
    ...
    byAgent: Object.fromEntries([...byAgent.entries()].map(([k, v]) => [k, agg(v)])),
...
function apiMemoryList(req, res) {
  const data = readMemory();
  json(res, { entries: memoryToEntries(data) });
}

function apiMemoryCreate(req, res) {
  let body = '';
  req.on('data', c => { body += c; });
  req.on('end', () => {
    let entry;
    try { entry = JSON.parse(body); } catch { return error(res, 400, 'некорректный JSON'); }
    if (!entry.key) return error(res, 400, 'поле key обязательно');
    if (entry.content !== undefined && entry.value === undefined) entry.value = entry.content;
    if (!entry.value) return error(res, 400, 'поле value/content обязательно');
    ...
    writeMemory(data);
    json(res, { key: entry.key, value: entry.value, tags: data[entry.key].tags, source: data[entry.key].source, created: now, updated: now }, 201);
  });
}

function apiMemoryUpdate(req, res, memoryKey) {
  let body = '';
  req.on('data', c => { body += c; });
  req.on('end', () => {
    let updates;
    ...
    const data = readMemory();
    if (!data[memoryKey]) return error(res, 404, 'запись не найдена');

    if (updates.content !== undefined && updates.value === undefined) updates.value = updates.content;

    const now = new Date().toISOString();
    if (updates.value !== undefined) data[memoryKey].value = updates.value;
    if (updates.tags !== undefined) data[memoryKey].tags = Array.isArray(updates.tags) ? updates.tags : [];
    if (updates.source !== undefined) data[memoryKey].source = updates.source;
    data[memoryKey].updated = now;

    writeMemory(data);
    json(res, {
      key: memoryKey,
      value: data[memoryKey].value,
      tags: data[memoryKey].tags,
      source: data[memoryKey].source,
      created: data[memoryKey].created,
      updated: data[memoryKey].updated,
    });
  });
}

function apiMemoryDelete(req, res, memoryKey) {
  const data = readMemory();
  if (!data[memoryKey]) return error(res, 404, 'запись не найдена');
  delete data[memoryKey];
  writeMemory(data);
  json(res, { ok: true });
}
...
function createServer() {
  const server = http.createServer((req, res) => {
    ...
    try {
      ...
      // memory
      if (p === '/api/memory') {
        if (req.method === 'GET') return apiMemoryList(req, res);
        if (req.method === 'POST') return apiMemoryCreate(req, res);
      }
      const memMatch = p.match(/^\/api\/memory\/(.+)$/);
      if (memMatch) {
        const memKey = decodeURIComponent(memMatch[1]);
        if (req.method === 'PUT') return apiMemoryUpdate(req, res, memKey);
        if (req.method === 'DELETE') return apiMemoryDelete(req, res, memKey);
      }
  ...
  // --- WebSocket server for live job updates ---
  const wss = new WebSocketServer({ server });

  let wssLastSnapshot = null;

  // Single shared polling interval for all WebSocket clients
  const wsPollInterval = setInterval(() => {
    // Only poll if there are connected clients
    if (wss.clients.size === 0) return;

    const currentJobs = emitJobsUpdate();
    const currentSnapshot = JSON.stringify(currentJobs);

    if (currentSnapshot !== wssLastSnapshot) {
      // Compute diffs
      const changes = [];
      if (wssLastSnapshot) {
        const lastJobs = JSON.parse(wssLastSnapshot);
        const currentMap = new Map(currentJobs.map(j => [j.id, j]));
        const lastMap = new Map(lastJobs.map(j => [j.id, j]));
        ...
        for (const [id, job] of currentMap) {
          const lastJob = lastMap.get(id);
          if (!lastJob || JSON.stringify(lastJob) !== JSON.stringify(job)) {
            changes.push({ type: 'job_updated', job });
          }
        }
        ...
        for (const [id] of lastMap) {
          if (!currentMap.has(id)) {
            changes.push({ type: 'job_deleted', jobId: id });
          }
        }
      } else {
        // First snapshot after a client connected — send all as updates
      ...
      wssLastSnapshot = currentSnapshot;
    }
  }, 1500); // Check every 1.5 seconds
  ...
  wss.on('connection', (ws) => {
    ...
    const currentJobs = emitJobsUpdate();
    wssLastSnapshot = JSON.stringify(currentJobs);
    const initData = JSON.stringify(currentJobs.map(j => ({ type: 'job_updated', job: j })));
    ws.send(initData);
  });

  // Cleanup polling on server close
  server.on('close', () => {
    clearInterval(wsPollInterval);
  });

  // Expose wss for testing
  server._wss = wss;
