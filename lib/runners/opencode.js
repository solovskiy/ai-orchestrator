...
module.exports = {
  ...
  parse(events) {
    ...
    let emptyStepCount = 0; // см. пояснение ниже
    const texts = [];
      ...
      switch (e.type) {
        ...
        case 'step_finish':
          ...
          if (part.tokens) {
            ...
            // Любое реальное обращение к модели тратит хотя бы входные
            // токены на промпт/контекст — total===0 физически невозможен
            // при состоявшемся API-вызове. Найдено на живом провале
            // 2026-07-27 (free-тир, шаг с reason:"unknown" и всеми
            // токенами по нулям после нормального первого шага).
            if ((part.tokens.total || 0) === 0) emptyStepCount += 1;
          } else {
            emptyStepCount += 1;
          }
          break;
    ...
    if (!error && stepFinishCount >= 1 && emptyStepCount > 0) {
      error = `opencode: ${emptyStepCount} из ${stepFinishCount} шагов вернулись с 0 токенов — ` +
        'пустой/несостоявшийся ответ API (типично для лимитов бесплатного тира), не решение модели';
    } else if (error && emptyStepCount > 0) {
      error = error + ` | дополнительно: ${emptyStepCount} из ${stepFinishCount} шагов с 0 токенов`;
    }
