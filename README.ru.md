...
Этот репозиторий никогда не хранит API-ключи — в нём только `agents/*.json`
(имена моделей, без учётных данных). Ключи живут в конфиге `opencode`,
полностью вне этого репозитория:

```bash
opencode auth login      # интерактивно: выбрать провайдера, вставить ключ
opencode auth list       # посмотреть, что уже настроено
```

Бесплатной дефолтной модели (`opencode/deepseek-v4-flash-free`, OpenCode
Zen) логин не нужен вообще. Платным провайдерам (OpenRouter и т.п.) —
`auth login` один раз на машину, дальше подхватывается автоматически
любым проектом, использующим эту установку `.ai` — ничего проектного
настраивать не нужно. Никогда не вставляй сырой ключ в промпт,
закоммиченный файл или в allowlist разрешений Claude Code
(`.claude/settings.local.json`) — этот файл в `.gitignore`, но остаётся
открытым текстом на диске; `opencode auth` держит ключ вообще вне
досягаемости этого репозитория.
...
`copywriter`), актуальный список с моделью/worktree/инструментами — команда, не
таблица:
