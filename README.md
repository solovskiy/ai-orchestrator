...
| `talk <id> "<текст>"` | реплика в живую `--interactive`-сессию, без ожидания ответа |
...
## Живой диалог (`--interactive` / `talk`)

`start --interactive` не завершает процесс после первого ответа — держит
его stdin открытым через именованный канал (`jobs/<id>/in.fifo`), и `talk
<id> "<текст>"` дописывает туда реплики без создания новой задачи. Нужно
для случая, когда агент по ходу длинной задачи что-то спрашивает
(«ставить библиотеку?») и ждать `send`-цикла (новая сессия → новый
`wait`) неудобно.

Это осознанно узкая надстройка, а не постоянно живущий RPC-канал (см.
«Про ACP» выше): диск остаётся источником правды. Если канал не
поднялся (не все файловые системы поддерживают FIFO) — `interactiveDegraded`
в `job.json`, задача тихо работает как обычная, `talk` откажет с
понятной причиной, а `send` по `sessionId` продолжает работать как обычно.
Сейчас поддерживает только `runner=opencode`.
